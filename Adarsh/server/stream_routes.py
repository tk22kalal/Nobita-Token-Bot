...
@routes.get(r"/stream/{token}")
async def direct_stream_redirect(request: web.Request):
    """Direct stream endpoint that redirects to the watch URL - iOS compatible"""
    try:
        token = request.match_info["token"]
        ...
        if not original_msg:
            return web.Response(text="❌ Original message not found", status=404)
        ...
        for attempt in range(max_retries):
            ...
            return web.Response(text="❌ Too many requests, please try again later", status=429)
        
        # Generate streaming URL with /watch/ prefix
        file_name = get_name(log_msg) or temp_data['file_name'] or "NEXTPULSE"
        file_hash = get_hash(log_msg)
        
        # Use base URL for stream links
        if Var.HAS_SSL:
            base_url = f"https://{Var.FQDN}/"
        else:
            base_url = f"http://{Var.FQDN}/"
        ...
        # Redirect to the watch URL (Plyr player page)
        raise web.HTTPFound(stream_link)
        
    except web.HTTPFound:
        raise
    except Exception as e:
        logging.error(f"Error in direct_stream_redirect: {e}")
        return web.Response(text=f"❌ Failed to generate stream link: {str(e)}", status=500)


@routes.get(r"/download/{token}")
async def direct_download_redirect(request: web.Request):
    """Direct download endpoint that redirects to the final download URL - iOS compatible"""
    try:
        token = request.match_info["token"]
        ...
        # Redirect to the actual download URL (works on iOS)
        raise web.HTTPFound(download_link)
        
    except web.HTTPFound:
        raise
    except Exception as e:
        logging.error(f"Error in direct_download_redirect: {e}")
        return web.Response(text=f"❌ Failed to generate download link: {str(e)}", status=500)
